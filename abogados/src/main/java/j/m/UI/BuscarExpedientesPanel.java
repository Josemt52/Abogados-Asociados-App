...
public class BuscarExpedientesPanel extends JPanel {
    private JTable table;
    private DefaultTableModel tableModel;
    private JTextField txtBuscar;
    private JButton btnEditarExpediente;

    public BuscarExpedientesPanel() {
        setLayout(new BorderLayout());
        setBackground(Color.WHITE);

        // Panel superior con campo de búsqueda y actualizar
        ...
        btnEditarExpediente = new JButton("✏ Editar Expediente");

        panelSuperior.add(lblBuscar);
        panelSuperior.add(txtBuscar);
        panelSuperior.add(btnActualizar);
        panelSuperior.add(btnEditarExpediente);
        ...
        // Evento para editar el expediente seleccionado
        btnEditarExpediente.addActionListener(e -> editarExpedienteSeleccionado());
    }
    ...
    /**
     * Abre el cuadro de diálogo para editar un expediente seleccionado.
     */
    private void editarExpedienteSeleccionado() {
        int fila = table.getSelectedRow();
        if (fila == -1) {
            JOptionPane.showMessageDialog(this, "Seleccione un expediente para editar.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        String[] opciones = {"Editar Estados", "Editar Todo el Expediente"};
        int seleccion = JOptionPane.showOptionDialog(this, "¿Qué desea editar?", "Opciones de Edición",
                JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, opciones, opciones[0]);

        if (seleccion == 0) {
            new EditarEstadosDialog(numeroExpediente);
        } else if (seleccion == 1) {
            new EditarExpedienteCompletoDialog(numeroExpediente);
        }
    }
}
